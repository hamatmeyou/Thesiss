  ...
  if (user.role === 'admin') {
    ...
    mainSection = `
      ...
      <div style="margin-top:20px;">
        ${renderRecentNotifications(user)}
      </div>`;
    ...
    mainSection = `
      <div class="dashboard-grid-2-1">
        <div>
          <div class="card" onclick="navigateTo('submissions')" style="cursor:pointer; height: 100%;">
            ...
            ${pendingSubs.length === 0
              ...
              : `<div class="table-wrap"><table>
                  <thead><tr><th>Title</th><th>Group</th><th>Type</th><th>Submitted</th></tr></thead>
                  <tbody>${pendingSubs.map(s => {
                    const g = MockDB.getGroupById(s.groupId);
                    return `<tr>
                      <td><div style="font-weight:500;">${s.title}</div></td>
                      <td style="font-size:0.85rem;color:var(--text-2);">${g ? g.title.slice(0, 25) + '…' : '—'}</td>
    ...
    mainSection = `
      <div class="dashboard-grid-2-1">
        <div class="card" onclick="navigateTo('groups')" style="cursor:pointer; height: 100%;">
          ...
          <div style="margin-top:20px; padding:20px; background:rgba(124,109,240,0.05); border-radius:var(--radius-sm); border:1px dashed var(--primary-2);">
            <div style="font-weight:600; color:var(--primary-2); margin-bottom:8px;">Quick Status</div>
            <div style="font-size:0.85rem; color:var(--text-2);">Total Groups: ${allGroups.length}</div>
            <div style="font-size:0.85rem; color:var(--text-2);">Approved: ${allGroups.filter(g=>g.status==='Approved').length}</div>
            <div style="font-size:0.85rem; color:var(--text-2);">In Progress: ${allGroups.filter(g=>g.status==='In Progress').length}</div>
          </div>
        </div>
        <div>
          ${renderRecentNotifications(user)}
    ...
    mainSection = `
      <div class="dashboard-grid-2-1">
        <div class="card">
          ...
          <div class="table-wrap">
            <table>
              <thead><tr><th>Group</th><th>Phase</th><th>Status</th></tr></thead>
              <tbody>
                ${recentGroups.map(g => `
                  <tr>
                    <td><div style="font-weight:500;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${g.title}</div></td>
                    <td>${g.phase}</td>
                    <td>${statusBadge(g.status)}</td>
                  </tr>`).join('')}
              </tbody>
        ...
        <div class="card" onclick="navigateTo('feedback')" style="cursor:pointer;">
          ...
          ${recentFb.length ? recentFb.map(f => `
            <div style="padding:10px 0;border-bottom:1px solid var(--glass-border);">
              <div style="font-size:0.82rem;color:var(--text-2);">${f.content.slice(0, 80)}…</div>
...
function renderRecentNotifications(user) {
  const notifs = MockDB.getNotificationsForUser(user.id).slice(0, 4);
  ...
  return `
    <div class="card" style="height: 100%;">
      ...
      <div style="display:flex; flex-direction:column; gap:0;">
        ${notifs.length === 0 
          ? '<div class="empty-state" style="padding:20px 0;"><p>No notifications</p></div>'
          : notifs.map(n => `
            <div style="padding:12px 0; border-bottom:1px solid var(--glass-border); cursor:pointer;" onclick="navigateTo('${n.link}')">
              <div style="display:flex; gap:10px; align-items:flex-start;">
                <span style="font-size:16px; opacity:0.8;">${icons[n.type] || '🔔'}</span>
                <div>
                  <div style="font-size:0.85rem; font-weight:600; line-height:1.2;">${n.title}</div>
                  <div style="font-size:0.75rem; color:var(--text-3); margin-top:3px;">${formatDate(n.createdAt)}</div>
                </div>
              </div>
            </div>
          `).join('')}
      </div>
      ${notifs.length > 0 ? `<button class="btn-sm btn-outline" style="width:100%; margin-top:12px;" onclick="toggleNotifications()">View All</button>` : ''}
    </div>
  `;
}
