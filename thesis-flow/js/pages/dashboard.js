  ...
  if (user.role === 'admin') {
    statsCards = `
      <div class="stats-grid">
        <div class="stat-card"><span class="stat-icon">👤</span><div class="stat-info"><div class="stat-value">${stats.totalStudents}</div><div class="stat-label">Total Students</div></div></div>
        <div class="stat-card"><span class="stat-icon">🎓</span><div class="stat-info"><div class="stat-value">${stats.totalAdvisers}</div><div class="stat-label">Faculty Advisers</div></div></div>
        <div class="stat-card"><span class="stat-icon">👥</span><div class="stat-info"><div class="stat-value">${stats.totalGroups}</div><div class="stat-label">Thesis Groups</div></div></div>
        <div class="stat-card"><span class="stat-icon">📄</span><div class="stat-info"><div class="stat-value">${stats.totalSubmissions}</div><div class="stat-label">Total Submissions</div></div></div>
        <div class="stat-card"><span class="stat-icon">⏳</span><div class="stat-info"><div class="stat-value">${stats.pendingSubmissions}</div><div class="stat-label">Pending Reviews</div></div></div>
        <div class="stat-card"><span class="stat-icon">📅</span><div class="stat-info"><div class="stat-value">${stats.upcomingSchedules}</div><div class="stat-label">Upcoming Schedules</div></div></div>
      </div>`;

    const recentGroups = MockDB.getAllGroups().slice(0, 4);
    mainSection = `
      <div class="card">
        <div class="card-header"><div class="card-title">📋 Recent Thesis Groups</div><button class="btn-sm btn-outline" onclick="navigateTo('groups')">View All</button></div>
    ...
    statsCards = `
      <div class="stats-grid">
        <div class="stat-card"><span class="stat-icon">👥</span><div class="stat-info"><div class="stat-value">${myGroups.length}</div><div class="stat-label">My Groups</div></div></div>
        <div class="stat-card"><span class="stat-icon">📄</span><div class="stat-info"><div class="stat-value">${mySubs.length}</div><div class="stat-label">Total Submissions</div></div></div>
        <div class="stat-card"><span class="stat-icon">⏳</span><div class="stat-info"><div class="stat-value">${pending}</div><div class="stat-label">Pending Reviews</div></div></div>
        <div class="stat-card"><span class="stat-icon">📅</span><div class="stat-info"><div class="stat-value">${upcoming}</div><div class="stat-label">Upcoming Schedules</div></div></div>
      </div>`;

    const pendingSubs = mySubs.filter(s => s.status === 'Pending').slice(0, 5);
    mainSection = `
      <div class="card">
        <div class="card-header"><div class="card-title">⏳ Submissions Awaiting Review</div><button class="btn-sm btn-outline" onclick="navigateTo('submissions')">View All</button></div>
      ...
      <div class="card" style="margin-top:20px;">
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:16px;">
          ${myGroups.map(g => {
            ...
            return `
            <div style="background:var(--bg-2); padding:16px; border-radius:var(--radius-sm); border:1px solid var(--glass-border);">
    ...
    statsCards = `
      <div class="stats-grid">
        <div class="stat-card"><span class="stat-icon">📄</span><div class="stat-info"><div class="stat-value">${subs.length}</div><div class="stat-label">Submissions</div></div></div>
        <div class="stat-card"><span class="stat-icon">✅</span><div class="stat-info"><div class="stat-value">${subs.filter(s=>s.status==='Approved'||s.status==='Reviewed').length}</div><div class="stat-label">Reviewed</div></div></div>
        <div class="stat-card"><span class="stat-icon">📅</span><div class="stat-info"><div class="stat-value">${upcoming.length}</div><div class="stat-label">Upcoming Events</div></div></div>
        <div class="stat-card"><span class="stat-icon">💬</span><div class="stat-info"><div class="stat-value">${feedbk.length}</div><div class="stat-label">Feedback Received</div></div></div>
      </div>`;
    ...
    mainSection = `
      ${group ? `
      <div class="card" style="background:linear-gradient(135deg,rgba(124,109,240,0.12),rgba(192,132,252,0.08));border-color:rgba(124,109,240,0.2);">
